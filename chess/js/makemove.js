    ...
    if(prPce != PIECES.EMPTY)   {
        if(LAST_MOVE == move){
            ...
            if(GameController.BoardFlipped == BOOL.TRUE) {
                $('.whiteChoice').css('top', global_y - 60)
                $('.whiteChoice').css('left', global_x - 60)
                $('.blackChoice').css('top', global_y)
                $('.blackChoice').css('left', global_x)
            }
            else{
                $('.whiteChoice').css('top', global_y)
                $('.whiteChoice').css('left', global_x)
                $('.blackChoice').css('top', global_y - 60)
                $('.blackChoice').css('left', global_x - 60)
            }
            if(side == COLOURS.WHITE){
            ...
            $('.piece-choice').on('click', function(e){
                new_piece = $(this).attr('data-id')
                ClearPiece(to);
                AddPiece(to, new_piece);
                if(GameController.BoardFlipped == BOOL.TRUE) {
                    to = MIRROR120(to);
                }
                RemoveGUIPiece(to);
                AddGUIPiece(to, new_piece)
                CheckResult()
                LAST_MOVE  = null
                CHOUSEN_PIECE = new_piece
                //console.log(to)
                $('.whiteChoice').hide()
                $('.blackChoice').hide()
                $('.hider').hide()
